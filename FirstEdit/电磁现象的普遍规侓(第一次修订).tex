...
[\textbf{界面处的切向磁场跃变}] $\vec{e_n}\times(\vec H_{2n}-\vec H_{1n})=\vec\alpha_f$, $(\oint_L\vec H\cdot d\vec l=\int_L\vec\alpha_f\cdot d\vec l+\frac{d}{dt}\int_S\vec D\cdot d\vec S$, 其中$\frac{\partial \vec D}{\partial t}$有限).\par
